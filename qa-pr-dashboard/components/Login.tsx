...
export default function Login({ apiUrl, onLoginSuccess }: LoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await login({ username: username.trim(), password }, apiUrl)
      onLoginSuccess?.()
    } catch (err: any) {
      console.error('Login error:', err)
      const errorMessage = err.message || 'Login failed. Please check your credentials and ensure the API server is running.'
